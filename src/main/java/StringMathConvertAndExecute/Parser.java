...
package StringMathConvertAndExecute;
...
public class Parser {
	...
	Expr expr = parseExpr(0);
        ...
        int tempI= tokens.getCountWordTokens();
        if( tempI!=1){
            throw error("More than one variable",
			SyntaxException.INCOMPLETE, null);
        }
        String tempS =tokens.getVariables();
        if(tempS.compareTo("y")!=0 && tempS.compareTo("Y")!=0){
            throw error("variable name Error",
			SyntaxException.INCOMPLETE, null);
        }
